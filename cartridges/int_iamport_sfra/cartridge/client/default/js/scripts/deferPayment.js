 ...
 * @param {Object} paymentResources payment resources
 ...
 */
const defer = function (item, callback, paymentResources, validationUrl, max, waitTime) {
	let iter = 0;
	waitTime = waitTime || 100;
	max = Math.max(max, 10);
	if (window[item] && typeof window[item] !== 'undefined') {
		callback(item, paymentResources, validationUrl);
	} else {
		if (max > iter) {
			iter += 1;
			setTimeout(function () {
				defer(item, callback, paymentResources, validationUrl, max, iter);
			}, waitTime);
		} else {
